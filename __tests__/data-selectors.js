...
import {selectData} from "../lib/graphql-selectors";
...
describe('Selectors', () => {
  describe('selectData', () => {
    ...
    it('should return root data when resource is an object', () => {
      ...
      expect(is(subject, fromJS({credentials: {token: 'mytoken'}}))).toBe(true);
    });

    it('should return root object when resource is an array but selected with id', () => {
      let subject = selectData(data, '{users(id: "2") {name}}');

      expect(is(subject, fromJS({users: {id: "2", name: 'Zanatta'}}))).toBe(true);
    });
  });
});
