  ...
  return new recordsTypes[type](
    Object.keys(entity).reduce(
      (red, key) => {
        ...
        if (typeof field == "object" && Array.isArray(field) == false) {
          console.log(
            "OBJ",
            graphQLSchema._typeMap[type]._fields[field.name].name
          );
          return Object.assign({}, red, {
              ...
              v =>
                typeof v == "object"
                  ? convertsEntityToRecord(
                      v,
                      graphQLSchema._typeMap[type]._fields[v.name].ofType.name,
                      graphQLSchema,
                      recordsTypes
                    )
                  : v
