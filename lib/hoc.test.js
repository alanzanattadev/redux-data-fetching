...
describe("HOC", () => {
  ...
  describe("DataHandlers", () => {
    ...
    it("dispatches mutations when a handler is called", () => {
      ...
      const onClickSpy = jest.fn(
        () => new Mutation({ mutationQL: "mutation {}" }),
      );
      const ConnectedComp = DataHandlers({
        mapMutationsToProps: () => ({
          onClick: onClickSpy,
      ...
      subject.find(Comp).at(0).prop("onClick")("a", "b");

      expect(spy).toHaveBeenCalled();
      expect(spy.mock.calls).toMatchSnapshot();
      expect(onClickSpy).toHaveBeenCalled();
      expect(onClickSpy.mock.calls).toMatchSnapshot();
    });
    ...
    it("passes the query progress and response as props", () => {
      function Comp({
        createUserQueryProgress,
        onClickQueryProgress,
        onClickQueryResponse,
      }) {
        expect(createUserQueryProgress).toMatchSnapshot();
        expect(onClickQueryProgress).toMatchSnapshot();
        expect(onClickQueryResponse).toMatchSnapshot();
        return null;
      }
      const ConnectedComp = DataHandlers({
        mapMutationsToProps: () => ({
  ...
  describe("GraphQLConnecter", () => {
    ...
    it("selects data in the cache", done => {
      ...
      const hash = hashString(request).toString();
      const User = compose(
