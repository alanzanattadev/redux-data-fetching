...
function getImmutableSelector(graphQLQuery: string): ImmutableSelector {
  ...
  let idArg;
  if (rootField.arguments.length > 0 && (idArg = rootField.arguments.find(arg => arg.name.value == "id"))) {
...
function convertIDsToIndex(data: Object, selector: ImmutableSelector): ImmutableSelector {
  return selector.reduce((reduction: {rootData: any, newSelector: any}, value: any) => {
    if (reduction.newSelector.count() > 0 && reduction.newSelector.last() === null)
      return reduction;
    else {
      if (typeof value == 'string' || typeof value == 'number') {
        ...
        if (value.type == 'id') {
          let selectedIndex = reduction.rootData.findIndex(item => item.get('id') == value.value);
          if (selectedIndex == -1)
          selectedIndex = null;
  ...
  }, {rootData: data, newSelector: List()}).newSelector.toJS();
}
