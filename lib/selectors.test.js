...
import {
  convertsTypesSchemaToSelectorSchema,
  convertsQueryToSelectorQuery,
} from "./selectors";
...
describe("Selectors", () => {
  ...
  describe("convertsTypesSchemaToSelectorSchema", () => {
    ...
    it("resolves to result of the needs with fragments", done => {
      const selectorSchema = convertsTypesSchemaToSelectorSchema(littleSchema);
      const queryHash = "hash";
      const User = Record({ id: null, name: null, friends: List() });
      const Progress = Record({ percent: 0, status: "WAITING" });
        ...
        `
          {
            ...
            females: users(gender: "female") {
              ...infos,
            }
          }
          
          fragment infos on UserSelector {
            id,
            name,
            friends {
              id,
  ...
  describe("convertsQueryToSelectorQuery", () => {
    it("converts name of fragment type in selector name", () => {
      expect(
        convertsQueryToSelectorQuery(`
          {
            users {
